...
seurat_script <<- "/home/project/Single_cells_project/scrna_project/pipeline/vdj_script/seurat_tcr_single"
scrna_project_path <<- "/home/project/Single_cells_project/scrna_project"
analysis_path <<- c("1_Cellranger", "2_Clonotype_analysis", "3_scrna_tcr_analysis")
...
source(file.path(seurat_script,'tcr_funcs.R'), encoding = 'UTF-8')
...
project_work_dir <- file.path(project_dir, paste(project_code, 'vdj', 'tcr', 'report_single', sep = "_"))
...
# samplelist <- setdiff(list.files(file.path(project_dir, 'data', 'tcr')),list.files(file.path(project_work_dir)))
samplelist <- list.files(file.path(project_dir, 'data', 'tcr'))
...
for (sample in samplelist){
  ...
  sample_data_src[sample] = file.path(project_dir, 'data', 'tcr', sample)
